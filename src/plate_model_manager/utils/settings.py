...
# 2. plate-model-manager-settings.toml in the current working directory, if it exists
# 3. settings.toml in ~/.plate-model-manager, if it exists
# If ~/.plate-model-manager/settings.toml does not exist, it is created with default values.

_DEFAULTS = {
    "enable_log_to_file": False,
    "log_file":"~/.plate-model-manager/pmm.log",
    "debug": False,
    "test_level": 0,
}
...
_ENV_VARS = {
    "debug": "PMM_DEBUG",
    "test_level": "PMM_TEST_LEVEL",
    "enable_log_to_file": "PMM_ENABLE_LOG_TO_FILE",
    "log_file": "PMM_LOG_FILE",
}

_PMM_HOME = Path("~/.plate-model-manager").expanduser()
_HOME_SETTINGS_FILE = _PMM_HOME / "settings.toml"
_CWD_SETTINGS_FILE = Path("plate-model-manager-settings.toml")
...
enable_log_to_file = _get_bool("enable_log_to_file")
log_file = str(Path(_get_str("log_file")).expanduser())
